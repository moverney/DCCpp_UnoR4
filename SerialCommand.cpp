 ...
 *    compared to Uno R3 the system clock is not changed by this command.
 *    To restore original pre-scaler (1) just send the same command
 */

    Serial.println("\nEntering Diagnostic Mode...");
    delay(1000);
    
    if (R_GPT6->GTCR_b.TPCS == 0) {
      R_GPT6->GTCR |= (0b101 << 24); // Pre-scaler 1024, TPCS[2:0] = 0b101 
      R_GPT2->GTCR |= (0b101 << 24); // Pre-scaler 1024, TPCS[2:0] = 0b101 
    }
    else {
      Serial.println("\nLeaving Diagnostic Mode...");
      R_GPT6->GTCR_b.TPCS = 0;
 ...
 */
      int v; 
      INTERFACE.print("<F is not supported on R4");
      break;
