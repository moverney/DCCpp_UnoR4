...
void RegisterList::readCV(char *s) volatile{
  ...
  for(int i=0;i<8;i++){
    ...
    for(int j=0;j<ACK_BASE_COUNT;j++) {
      int val = (int)analogRead(CURRENT_MONITOR_PIN_PROG);
      base += val;
    }
    base/=ACK_BASE_COUNT;
    ...
    //loadPacket(0,resetPacket,2,6);          // forces code to wait until all repeats of bRead are completed (and decoder begins to respond)
